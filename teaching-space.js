(function () {
  ...
  function renderContent() {
    ...
    body.innerHTML = `<div class="teach-card"><div class="teach-row"><label for="chapterTitle">Nouveau chapitre</label><input id="chapterTitle" maxlength="180" placeholder="Ex. Installation électrique d’un logement"><button type="button" id="addChapter">Créer le chapitre</button></div></div>
      <div class="teach-list">${chapters.map(ch => `<details open><summary>${esc(ch.title)} <span class="teach-meta">${ch.published ? 'Publié' : 'Brouillon'}</span></summary><div class="teach-row"><button type="button" data-publish-chapter="${ch.id}" class="subtle">${ch.published ? 'Masquer aux élèves' : 'Publier le chapitre'}</button><button type="button" data-add-item="${ch.id}">Ajouter ${esc(kindNames[tab]).toLowerCase()}</button></div>${items.filter(i => i.chapter_id === ch.id && i.kind === tab).map(i => `<div class="teach-item"><div><strong>${esc(i.title)}</strong><small class="teach-meta">${i.published ? 'Publié' : 'Brouillon'}${i.linked_course_id ? ' · lié à un cours' : ''}</small></div><div class="teach-row"><button type="button" data-edit-item="${i.id}" class="subtle">Modifier</button><button type="button" data-publish-item="${i.id}" class="subtle">${i.published ? 'Masquer' : 'Publier'}</button></div></div>`).join('') || '<p>Aucun contenu dans cette rubrique.</p>'}</details>`).join('') || '<div class="teach-card">Aucun chapitre pour cette classe.</div>'}</div><div id="teachEditor"></div>`;
  ...
  async function editItem(id, chapterId) {
    ...
    host.innerHTML = `<div class="teach-card teach-editor"><h2>${editing ? 'Modifier' : 'Créer'} : ${esc(kindNames[tab])}</h2><label>Titre <input id="itemTitle" maxlength="180" value="${esc(editing?.title || '')}" required></label>${tab === 'td' ? `<label>Cours lié <select id="linkedCourse"><option value="">Aucun</option>${courseOptions.map(i => `<option value="${i.id}" ${editing?.linked_course_id === i.id ? 'selected' : ''}>${esc(i.title)}</option>`).join('')}</select></label>` : ''}<div id="blockList"></div><div class="teach-row">${tab !== 'tp' ? '<button type="button" id="addText" class="subtle">＋ Texte / tableau</button><button type="button" id="addVideo" class="subtle">＋ Vidéo</button>' : ''}<button type="button" id="addFile" class="subtle">＋ Image ou document</button><input id="itemFile" type="file" accept="image/*,.pdf,.doc,.docx" hidden></div><div class="teach-row"><button type="button" id="saveItem">Enregistrer</button><button type="button" id="cancelItem" class="subtle">Annuler</button></div><p class="teach-help">Publiez ensuite le chapitre et le contenu pour les rendre accessibles aux élèves.</p></div>`;
  ...
  async function saveItem(chapterId) {
    ...
    try {
      ...
      for (const block of blocks.filter(b => b.type === 'pending')) {
      ...
      await api.rest('learning_items?id=eq.' + id, {method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({blocks:payload.blocks})});
      activeItem = null; await loadClass(); status('Contenu enregistré en brouillon.');
