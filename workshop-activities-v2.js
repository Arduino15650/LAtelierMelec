...
(function(){
  ...
  renderActivities=function(){
    ...
    $('#activitiesView').innerHTML=`<div class="page-head activities-page-head"><div><h1>Activités en atelier</h1><p class="activities-description">Activités attribuées à un ou plusieurs élèves, créées et évaluées selon le Référentiel MELEC 2024.</p><p class="official-reference">Arrêté du 1er mars 2024 — Bac Pro MELEC • JORF n°57 du 8 mars 2024</p></div></div>${activities.length?`<div class="activity-class-list">${classes.map(className=>{const items=activities.filter(a=>(a.className||'Classe non précisée')===className).sort((a,b)=>(b.date||'').localeCompare(a.date||'')||a.title.localeCompare(b.title,'fr'));return `<details class="activity-class-group"><summary><div><strong>${esc(className)}</strong><span>${items.length} activité(s)</span></div></summary><div class="activity-class-content">${items.map(activityCard).join('')}</div></details>`}).join('')}</div>`:`<div class="panel empty"><div class="symbol">▦</div><h2>Aucune activité en atelier</h2><p>Créez une activité puis choisissez les élèves auxquels elle est destinée.</p></div>`}<div class="stats bottom-stats"><div class="stat"><strong>${activities.length}</strong><span>activité(s) enregistrée(s)</span></div><div class="stat"><strong>${done}</strong><span>activité(s) avec évaluation(s)</span></div><button class="stat new-activity-stat" onclick="newActivity()"><span class="plus">+</span><div><strong>Nouvelle activité</strong><span>Créer pour un ou plusieurs élèves</span></div></button></div>`;
  };
