(function(){
  ...
  const availableClasses=()=>[...new Set([...classNames(),...state.studentGroups.map(g=>g.className).filter(Boolean)])].sort((a,b)=>a.localeCompare(b,'fr',{numeric:true}));
...
section.innerHTML=`<h2 class="panel-title">Groupes d’élèves</h2><div class="group-form-grid"><div class="field"><label for="studentGroupClass">Classe</label><select id="studentGroupClass"><option value="">Choisir une classe…</option>${availableClasses().map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join('')}</select></div><div class="field"><label for="studentGroupName">Nom du groupe</label><input id="studentGroupName" maxlength="80" placeholder="Ex. Groupe A"></div></div><details id="newGroupMembers" class="selection-dropdown" hidden><summary>Choisir les élèves du groupe</summary><div id="studentGroupChoices" class="group-member-list"></div></details><button type="button" id="createStudentGroup" class="button primary">Créer le groupe</button><div id="studentGroupsList" class="student-groups-list"></div>`;
    ...
    const update=()=>{const name=classSelect.value,students=members(name);fold.hidden=!name;choice.innerHTML=choices(students)||'<p>Aucun élève dans cette classe.</p>';list.innerHTML=name?`<h3>Groupes de ${esc(name)}</h3>${groups(name).map(g=>{const selected=students.filter(s=>g.studentIds.includes(s.id)),others=students.filter(s=>!g.studentIds.includes(s.id));return `<details class="student-group-item" data-group-id="${esc(g.id)}"><summary><strong>${esc(g.name)}</strong><span>${selected.length} élève(s)</span></summary><div class="student-group-item-body"><details class="selection-dropdown"><summary>Élèves du groupe</summary><ul class="group-name-list">${selected.map(s=>`<li>${esc(s.name)}</li>`).join('')}</ul></details><details class="selection-dropdown"><summary>Modifier les membres</summary><p class="group-edit-hint">Cochez les élèves à conserver, puis enregistrez. Aucune case n’est présélectionnée.</p><div class="group-member-list">${choices(selected)}</div>${others.length?`<div class="field"><label>Ajouter un élève</label><select class="group-add-student"><option value="">Choisir…</option>${others.map(s=>`<option value="${esc(s.id)}">${esc(s.name)}</option>`).join('')}</select></div>`:''}</details><div class="group-item-actions"><button type="button" class="button small group-save">Enregistrer</button><button type="button" class="button small danger group-delete">Supprimer</button></div></div></details>`}).join('')||'<p>Aucun groupe créé.</p>'}`:'';showStudentGroups()};
    ...
    list.onclick=e=>{const row=e.target.closest('[data-group-id]'),g=state.studentGroups.find(x=>x.id===row?.dataset.groupId);if(!g)return;if(e.target.closest('.group-save')){const ids=[...row.querySelectorAll('.group-member-list input:checked')].map(x=>x.value),added=row.querySelector('.group-add-student')?.value;if(added)ids.push(added);if(!ids.length)return toast('Sélectionnez au moins un élève.');g.studentIds=[...new Set(ids)];save();update();toast('Groupe mis à jour. Les élèves déjà associés aux activités sont conservés.')}if(e.target.closest('.group-delete')&&confirm(`Supprimer le groupe « ${g.name} » ? Le lien avec les activités sera retiré, mais les activités et évaluations resteront enregistrées.`)){state.studentGroups=state.studentGroups.filter(x=>x.id!==g.id);state.activities.filter(a=>a.groupId===g.id).forEach(a=>delete a.groupId);save();update();toast('Groupe supprimé. Vérifiez la synchronisation.')}};
  };
