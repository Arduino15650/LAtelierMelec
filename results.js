(function(){
  ...
  let selectedClass='';
  let selectedGroup='';
  let selectedStudentIds=null; // null = classe ou groupe entier
  ...
  function renderResults(keepStudentListOpen=false){
    ...
    if(selectedClass&&!classes.includes(selectedClass)){selectedClass='';selectedGroup='';selectedStudentIds=null}
    const groups=groupList(selectedClass);
    if(selectedGroup&&!groups.some(group=>group.id===selectedGroup)){selectedGroup='';selectedStudentIds=null}
    ...
    const selected=selectedStudentIds===null?pupils:pupils.filter(student=>selectedStudentIds.includes(student.id));
    ...
    target.querySelector('#resultsClass')?.addEventListener('change',event=>{selectedClass=event.target.value;selectedGroup='';selectedStudentIds=null;renderResults()});
    target.querySelector('#resultsGroup')?.addEventListener('change',event=>{selectedGroup=event.target.value;selectedStudentIds=null;renderResults()});
